...
class MetadataScrambler
{
    ...
    public function generateRandomMetadata()
    {
        $metadata = [
    ...
    private function processFileWithHashObfuscation($inputPath, $outputPath, $fileType, $metadata)
    {
        switch ($fileType) {
            ...
            case 'application/vnd.openxmlformats-officedocument.wordprocessingml.document':
                ...
                break;
            default:
                $this->writeLog("Unsupported file type in processFileWithHashObfuscation: $fileType for $inputPath");
                $result = false;
                break;
        }
    ...
    private function scrambleWebp($inputPath, $outputPath, $metadata)
    {
        ...
        try {
            ...
            // Apply hash obfuscation if enabled
            if ($this->options['obfuscate_hashes']) {
                $methods = $this->options['hash_obfuscation_methods'] ?? ['noise', 'artifacts'];
                foreach ($methods as $method) {
                    switch ($method) {
                        case 'noise':
            ...
            // Save WebP with high quality to strip existing metadata
            $quality = $this->options['preserve_quality'] ? ($this->options['jpeg_quality'] ?? 95) : 90;
            ...
            // Add XMP metadata instead of EXIF
            if (!$this->options['strip_only'] && $metadata) {
                if (!$this->addWebpXmp($outputPath, $metadata)) {
                    $this->writeLog("Failed to add XMP metadata to $outputPath");
                    return false;
                }
            }
    ...
    private function addWebpXmp($filePath, $metadata)
    {
        $content = file_get_contents($filePath);
        if ($content === false || substr($content, 0, 4) !== "RIFF" || substr($content, 8, 4) !== "WEBP") {
            $this->writeLog("Invalid WebP header in $filePath");
            return false;
        }
        ...
        // Remove existing XMP or EXIF chunks
        $newContent = substr($content, 0, 12); // Keep RIFF header and WEBP identifier
        $offset = 12;
        while ($offset < strlen($content)) {
            if ($offset + 8 > strlen($content)) {
                $this->writeLog("Incomplete WebP chunk at offset $offset");
                return false;
            }
            $chunkType = substr($content, $offset, 4);
            $chunkSize = unpack('V', substr($content, $offset + 4, 4))[1];
            if ($chunkType === 'EXIF' || $chunkType === 'XMP ') {
                $this->writeLog("Removed $chunkType chunk at offset $offset, size $chunkSize");
                $offset += 8 + $chunkSize + ($chunkSize % 2); // Skip chunk + padding
            } else {
                $newContent .= substr($content, $offset, 8 + $chunkSize + ($chunkSize % 2));
                $offset += 8 + $chunkSize + ($chunkSize % 2);
            }
        }

        // Insert XMP chunk after VP8/VP8L/VP8X chunk
        $vp8Pos = strpos($newContent, 'VP8', 12);
        if ($vp8Pos === false) {
            $this->writeLog("No VP8/VP8L/VP8X chunk found in $filePath");
            return false;
        }
        $vp8Size = unpack('V', substr($newContent, $vp8Pos + 4, 4))[1];
        $insertPos = $vp8Pos + 8 + $vp8Size + ($vp8Size % 2);

        // Create XMP chunk
        $xmpChunk = 'XMP ' . pack('V', strlen($xmp)) . $xmp;
        if (strlen($xmp) % 2) {
            $xmpChunk .= "\x00"; // Padding for odd-sized chunk
        }

        // Combine content
        $finalContent = substr($newContent, 0, $insertPos) . $xmpChunk . substr($newContent, $insertPos);
        // Update RIFF size
        $newSize = strlen($finalContent) - 8;
        $finalContent = substr_replace($finalContent, pack('V', $newSize), 4, 4);

        if (file_put_contents($filePath, $finalContent) === false) {
            $this->writeLog("Failed to write WebP XMP data to $filePath");
            return false;
        }

        $this->writeLog("Added XMP metadata to WebP $filePath");
        return true;
    }
    ...
    private function scrambleDocx($inputPath, $outputPath, $metadata)
    {
        ...
        try {
            ...
            $coreExists = $zip->locateName('docProps/core.xml') !== false;
            $appExists = $zip->locateName('docProps/app.xml') !== false;
            ...
            for ($i = 0; $i < $zip->numFiles; $i++) {
                ...
                if ($content === false) {
                    ...
                    $newZip->close();
                    $zip->close();
                    return false; // Stop processing to avoid corrupt output
                }
                ...
                if ($filename === 'docProps/core.xml' && !$this->options['strip_only'] && $metadata) {
                    ...
                    $this->writeLog("Skipping custom.xml due to strip_only option");
                    continue;
                } elseif ($filename === 'docProps/custom.xml' && !$this->options['strip_only'] && $metadata) {
                    $content = $this->createDocxCustomProperties($metadata);
                }

                if (!$newZip->addFromString($filename, $content)) {
                    $this->writeLog("Failed to add file to new DOCX: $filename");
                    $newZip->close();
                    $zip->close();
                    return false;
                }
            ...
            // Add custom.xml if it didn't exist and strip_only is false
            if (!$this->options['strip_only'] && $metadata && !$zip->locateName('docProps/custom.xml')) {
                $customContent = $this->createDocxCustomProperties($metadata);
                if (!$newZip->addFromString('docProps/custom.xml', $customContent)) {
                    $this->writeLog("Failed to add new custom.xml to DOCX");
                    $newZip->close();
                    $zip->close();
                    return false;
                }
            ...
            $zip->close();
            if (!$newZip->close()) {
                $this->writeLog("Failed to finalize DOCX: $outputPath");
                return false;
            }

            // Validate output
            if ($this->options['validate_output'] && !$this->validateOutput($outputPath, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')) {
                $this->writeLog("DOCX output validation failed: $outputPath");
                return false;
            }
            ...
            $this->writeLog("DOCX processing failed for $inputPath: " . $e->getMessage());
            return false;
        }
    }
...
<vt:lpwstr>' . $gps . '</vt:lpwstr>
    ...
    private function setWindowsTime($timestamp)
    {
        ...
        if ($this->options['system_time_method'] === 'powershell' || $this->options['system_time_method'] === 'auto') {
            $methods[] = "powershell -Command \"Set-Date -Date '$dateStr $timeStr'\"";
        }
        if ($this->options['system_time_method'] === 'date' || $this->options['system_time_method'] === 'auto') {
            $methods[] = "date $dateStr && time $timeStr";
        }
        ...
        foreach ($methods as $cmd) {
            ...
            if ($returnCode === 0) {
                $this->writeLog("Windows time set via command: $cmd");
                return true;
            } else {
                $this->writeLog("Windows time setting failed for command '$cmd': " . implode(' ', $output));
            }
        }
    ...
    private function setLinuxTime($timestamp)
    {
        ...
        if ($this->options['system_time_method'] === 'hwclock' || $this->options['system_time_method'] === 'auto') {
            $methods[] = "hwclock --set --date '$dateString'";
        }
        ...
        foreach ($methods as $cmd) {
            ...
            if ($returnCode === 0) {
                $this->writeLog("Linux time set via command: $cmd");
                return true;
            } else {
                $this->writeLog("Linux time setting failed for command '$cmd': " . implode(' ', $output));
            }
        }
    ...
    {
        $dateString = date('mdHisY', $timestamp);
        $cmd = "sudo date $dateString";
        $output = [];
        $returnCode = 0;
        exec($cmd . ' 2>&1', $output, $returnCode);
        if ($returnCode === 0) {
            $this->writeLog("MacOS time set: $dateString");
            return true;
        } else {
            $this->writeLog("MacOS time setting failed: " . implode(' ', $output));
            return false;
        }
    }
    ...
    private function scrambleFileSystemTimestamps($filePath, $metadata)
    {
        if ($this->options['use_same_timestamp']) {
            ...
            $timestamp = mt_rand(strtotime('2000-01-01'), time());
        }

        if ($timestamp === false) {
            $this->writeLog("Invalid timestamp for file system metadata: {$metadata['date']}");
            return false;
        }

        if (!touch($filePath, $timestamp, $timestamp)) {
            $this->writeLog("Failed to set filesystem timestamps for $filePath");
            return false;
        }

        $this->writeLog("Set filesystem timestamps for $filePath to " . date('Y-m-d H:i:s', $timestamp));
        return true;
    }
    ...
    private function detectFileType($filePath)
    {
        if (!file_exists($filePath)) {
            $this->writeLog("Cannot detect file type: File does not exist: $filePath");
            return false;
        }
        ...
        $this->writeLog("Detected MIME type for $filePath: $mimeType");
        return $mimeType;
    }

    private function validateOutput($filePath, $expectedMimeType)
    {
        if (!file_exists($filePath)) {
            $this->writeLog("Output validation failed: File does not exist: $filePath");
            return false;
        }
        ...
        if ($actualMimeType !== $expectedMimeType) {
            $this->writeLog("MIME type mismatch: Expected $expectedMimeType, got $actualMimeType for $filePath");
            return false;
        }
        ...
        if ($expectedMimeType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
            ...
            if ($zip->open($filePath) !== true) {
                $this->writeLog("DOCX validation failed: Not a valid ZIP archive: $filePath");
                return false;
            }
            if ($zip->locateName('word/document.xml') === false) {
                $this->writeLog("DOCX validation failed: Missing word/document.xml in $filePath");
                $zip->close();
                return false;
            }
            $zip->close();
        } elseif (in_array($expectedMimeType, ['image/jpeg', 'image/png', 'image/gif', 'image/webp'])) {
            $content = file_get_contents($filePath);
            if ($content === false) {
                $this->writeLog("Failed to read file for validation: $filePath");
                return false;
            }
            if ($expectedMimeType === 'image/jpeg' && substr($content, 0, 2) !== "\xFF\xD8") {
                $this->writeLog("Invalid JPEG header in $filePath");
                return false;
            } elseif ($expectedMimeType === 'image/png' && substr($content, 0, 8) !== "\x89PNG\x0D\x0A\x1A\x0A") {
                $this->writeLog("Invalid PNG header in $filePath");
                return false;
            } elseif ($expectedMimeType === 'image/gif' && substr($content, 0, 6) !== 'GIF89a' && substr($content, 0, 6) !== 'GIF87a') {
                $this->writeLog("Invalid GIF header in $filePath");
                return false;
            } elseif ($expectedMimeType === 'image/webp' && (substr($content, 0, 4) !== "RIFF" || substr($content, 8, 4) !== "WEBP")) {
                $this->writeLog("Invalid WebP header in $filePath");
                return false;
            }
        }
    ...
    public function scramble($inputPath, $outputPath, $fileType = null)
    {
        ...
        if ($fileType === false) {
            $this->writeLog("Unable to determine file type for $inputPath");
            return false;
        }

        $this->writeLog("Starting scramble for $inputPath, Type: $fileType");
        $metadata = $this->options['strip_only'] ? null : $this->generateRandomMetadata();

        // Use scrambleWithHashVerification for all file types to ensure hash obfuscation
        return $this->scrambleWithHashVerification($inputPath, $outputPath, $fileType);
    }
}
?>